...
/* Helper for treedisk_setsize. Return the number of levels needed to support
 * nblocks, i.e ceil(log_RPB (nblocks)) + 1. 
 */
static unsigned int compute_levels(struct treedisk_snapshot snapshot) {
...
/* Helper for treedisk_setsize. Put a single block the_root onto the freelist.
 * We first check the superblock to see if there is already a freelist, and if not, we use the_root as the
 * freelist block and let the superblock points to it. If there is already a freelist block, then traverse the
 * first freelist block to look for zero and use that slot for the_root. If there is no zero, then again we use
 * the_root to create a new freelist block and let the superblock points to it.
 */
static void put_onto_freelist(struct treedisk_state *ts, struct treedisk_snapshot snapshot, block_no the_root) {

    block_no the_freelist_no = snapshot.superblock.superblock.free_list;

    // superblock.freelist == 0
    if (the_freelist_no == 0) {

        // prepare a new freelist block
        struct treedisk_freelistblock new_freelist_block;
        memset(new_freelist_block.refs, 0, REFS_PER_BLOCK*sizeof(new_freelist_block.refs[0]));

        // update superblock
        snapshot.superblock.superblock.free_list = the_root;

        // write to disk
        if ((*ts->below->write)(ts->below, the_root, (block_t *) &new_freelist_block) < 0) {
            panic("treedisk_write: new first freelist block");
        }
        if ((*ts->below->write)(ts->below, (block_no) 0, (block_t *) &snapshot.superblock.superblock) < 0) {
            panic("treedisk_write: superblock");
        }
    }

    // have at least a freelist block
    else {

        // read the first free list block, i.e the one superblock points to
        struct treedisk_freelistblock the_freelist_block;
        if ((*ts->below->read)(ts->below, the_freelist_no, (block_t *) &the_freelist_block) < 0) {
            panic("treedisk_read: first freelist block");
        }

        // traverse the first freelist block for 0, by Piazza @1552, zeros are only in first freelist block
        unsigned int i;
        unsigned int found = 0;
        for (i = 1; i < REFS_PER_BLOCK; i++) {
            if (the_freelist_block.refs[i] == 0) {
                found = i;
                break;
            }
        }
        ...
        if (found > 0) {
            ...
            the_freelist_block.refs[i] = the_root;

            // write back modified blocks
            if ((*ts->below->write)(ts->below, the_freelist_no, (block_t *) &the_freelist_block) < 0) {
                panic("treedisk_write: first freelist block");
            }
        }

        // did not find any 0, have to create a new freelist block
        else {

            // prepare a new freelist block
            struct treedisk_freelistblock new_freelist_block;
            memset(new_freelist_block.refs, 0, REFS_PER_BLOCK*sizeof(new_freelist_block.refs[0]));
            new_freelist_block.refs[0] = the_freelist_no;
            ...
            // write
            if ((*ts->below->write)(ts->below, the_root, (block_t *) &new_freelist_block) < 0) {
                panic("treedisk_write: new freelist block");
            }
            if ((*ts->below->write)(ts->below, (block_no) 0, (block_t *) &snapshot.superblock.superblock) < 0) {
                panic("treedisk_write: superblock");
            }
...
/* A recursive function that puts all blocks points by the_root onto the freelist. This
 * includes all indirect blocks and data blocks.
 */
 static void put_all_onto_freelist(struct treedisk_state *ts, struct treedisk_snapshot snapshot, block_no the_root, unsigned int nlevels) {

    // base case
    if (nlevels == 1) {
        put_onto_freelist(ts, snapshot, the_root);
        ...
        // read the root
        struct treedisk_indirblock the_root_block;
        if ((*ts->below->read)(ts->below, the_root, (block_t *) &the_root_block) < 0) {
            panic("treedisk_read: root data block");
        }

        // put all non-hole blocks onto freelist
        unsigned int i;
        for (i = 0; i < REFS_PER_BLOCK; i ++) {

            if (the_root_block.refs[i] > 0) {

                put_all_onto_freelist(ts, snapshot, the_root_block.refs[i], nlevels-1);
                treedisk_get_snapshot(&snapshot, ts->below, ts->inode_no);

            }
        }

        // put itself onto freelist
        put_onto_freelist(ts, snapshot, the_root);
        treedisk_get_snapshot(&snapshot, ts->below, ts->inode_no);
    }
 }
...
static int treedisk_setsize(block_store_t *this_bs, block_no nblocks){
    ...
    block_no the_root = snapshot.inode->root;

    // compute levels
    unsigned int nlevels = compute_levels(snapshot);

    // put all onto freelist
    put_all_onto_freelist(ts, snapshot, the_root, nlevels);
    treedisk_get_snapshot(&snapshot, ts->below, ts->inode_no);

    // get old inode size, and set it to 0 and writeback
    block_no old_size;
    old_size = snapshot.inode->nblocks;
    snapshot.inode->nblocks = 0;
    snapshot.inode->root = 0;
    if ((*ts->below->write)(ts->below, snapshot.inode_blockno, (block_t *) &snapshot.inodeblock.inodeblock) < 0) {
